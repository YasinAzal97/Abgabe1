...
import { Magazin } from '../../magazin/entity/magazin.entity.js';
// eslint-disable-next-line sort-imports
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Schlagwort } from '../../magazin/entity/schlagwort.entity.js';
import { magazine } from './testdaten.js';
// eslint-disable-next-line sort-imports
...
export class DbPopulateService implements OnApplicationBootstrap {
    ...
    /**
     * Initialisierung durch DI mit `Repository<Magazin>` gemäß _TypeORM_.
     */
    ...
    async #populatePostgres() {
        const schema = Magazin.name.toLowerCase();
        this.#logger.warn(
        ...
        const saved = await this.#repo.save(this.#magazine);
        this.#logger.warn(
    ...
    async #populateMySQL() {
        ...
        tabelle = Magazin.name.toLowerCase();
        this.#logger.warn(
            `${typeOrmModuleOptions.type}: Tabelle ${tabelle} wird geloescht`,
        );
        await this.#repo.query(
            `DROP TABLE IF EXISTS ${Magazin.name.toLowerCase()};`,
        );
        ...
        let createScript = resolve(scriptDir, 'create-table-magazin.sql');
