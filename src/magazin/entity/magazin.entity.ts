 ...
 */
export type Verlag = 'A_VERLAG' | 'B_VERLAG';

/**
 * Alias-Typ für gültige Strings bei der Art eines Magazines.
 */
export type MagazinArt = 'DIGITAL' | 'DRUCKAUSGABE';
...
export class Magazin {
    ...
    readonly art: MagazinArt | undefined;

    @Column('varchar')
    @ApiProperty({ example: 'A_VERLAG', type: String })
    ...
    @Column('varchar')
    @ApiProperty({ example: '1430-855X', type: String })
    ...
    @OneToMany(() => Schlagwort, (schlagwort) => schlagwort.magazin, {
...
export const removeissnDash = (magazin: Magazin) => {
    // https://www.typescriptlang.org/docs/handbook/2/mapped-types.html#mapping-modifiers
    const copy = magazin as {
        -readonly [K in keyof Magazin]: Magazin[K]; // eslint-disable-line no-use-before-define
    };
    copy.issn = magazin.issn.replaceAll('-', '');
    return copy;
};
