...
/**
 * Das Modul besteht aus der Klasse {@linkcode MagazinValidationService}.
...
import { type Magazin } from '../entity/magazin.entity';
...
@Injectable()
export class MagazinValidationService {
    #ajv = new Ajv2020({
        allowUnionTypes: true,
        allErrors: true,
    });

    readonly #logger = getLogger(MagazinValidationService.name);

    constructor() {
        // https://github.com/ajv-validator/ajv-formats#formats
        formatsPlugin(this.#ajv, ['date', 'email', 'uri']);
        ajvErrors(this.#ajv);
        this.#ajv.addFormat('issn', {
            type: 'string',
            validate: this.#validateissn,
        });
    }
    ...
    #checkChars(chars: string[]) {
        ...
        if (chars.length === 9) {
            // Compute the issn-10 check digit
            chars.reverse();
            ...
            // Compute the issn-13 check digit
            for (let i = 0; i < chars.length; i++) {
    ...
    #validateissn: FormatValidator<string> = (subject: string) => {
        // Checks for issn-10 or issn-13 format
        ...
        const regex =
            /^(?:issn(?:-1[03])?:? )?(?=[\dX]{10}$|(?=(?:\d+[- ]){3})[- \dX]{13}$|97[89]\d{10}$|(?=(?:\d+[- ]){4})[- \d]{17}$)(?:97[89][- ]?)?\d{1,5}[- ]?\d+[- ]?\d+[- ]?[\dX]$/u; //NOSONAR
        ...
        if (regex.test(subject)) {
            ...
            const chars = subject
                .replace(/[ -]|^issn(?:-1[03])?:?/gu, '')
                .split(''); // eslint-disable-line unicorn/prefer-spread
            // Remove the final issn digit from `chars`, and assign it to `last`
     ...
     */
    validate(magazin: Magazin) {
        this.#logger.debug('validate: magazin=%o', magazin);
        const validate = this.#ajv.compile<Magazin>(jsonSchema);
        validate(magazin);
