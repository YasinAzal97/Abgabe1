...
import { type MagazinModel } from '../../src/magazin/rest/magazin-get.controller.js';
...
describe('GET /:id', () => {
    ...
    each(idVorhanden).test(
        ...
        async (id: string) => {
            ...
            // when
            const response: AxiosResponse<MagazinModel> = await client.get(url);
    ...
    each(idNichtVorhanden).test(
        'Kein Magazin zu nicht-vorhandener ID %s',
        async (id: string) => {
            // given
            const url = `/${id}`;
    ...
    each(idVorhandenETag).test(
        'Magazin zu vorhandener ID %s mit ETag %s',
