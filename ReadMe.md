...
> `create-db-magazin.sh` und `create-db-magazin.sql` aus dem Verzeichnis
...
> `C:\Zimmermann\volumes\postgres\tablespace\magazin` muss außerdem Vollzugriff
...
> Datenbank-User `magazin` und dessen Datenbank `magazin` anlegen, d.h. der neue
> Datenbank-User `magazin` wird zum Owner der Datenbank `magazin`. Dazu muss man
...
> ❗ Vor dem 1. Start von MySQL muss man die Skripte `create-db-magazin.sh` und
> `create-db-magazin.sql` aus dem Projektverzeichnis
...
> `magazin` und dessen Datenbank `magazin` anlegen, d.h. der neue Datenbank-User
> `magazin` wird zum Owner der Datenbank `magazin`. Dazu muss man sich mit dem
...
`extras\restclient\graphql\magazin.query.http` und
`extras\restclient\graphql\magazin.mutation.http`.

In der Sandbox kann man z.B. folgende Query absetzen:

...
{
  magazin(id: "000000000000000000000001") {
    titel
    art
    issn
    version
...
Oder unter Verwendung von einer Variablen (hier: `magazinId`):

...
query ($magazinId: ID!) {
  magazin(id: $magazinId) {
    titel
    art
    issn
    version
...
...
{
  "magazinId": "000000000000000000000001"
}
...
...
```powershell
    npm exec jest --detectOpenHandles --errorOnDeprecated `
      --forceExit --runTestsByPath '__tests__\magazin\magazin-get.controller.test.ts'
```
...
wird das Docker-Image `docker.io/juergenzimmermann/magazin:1.0.0` mit dem implizit
...
```powershell
    pack inspect juergenzimmermann/magazin:1.0.0
```
...
- `helm install magazin . -f values.yaml -f dev.yaml` in `extras\helm`
...
welches durch _Mounting_ dem Windows-Verzeichnis `C:\Zimmermann\volumes\magazin`
...
- `helm uninstall magazin` in `extras\helm`
...
- "Feature Filenames", z.B. magazin.service.ts
